...
from src.pp.utils.visualisation.fonts import PINGFANG_FONT
from src.pp.common import (
    BaseCVResult,
    HtmlMixin,
    JsonMixin,
    MarkdownMixin,
...
from src.pp.pipelines.layout_parsing.utils import get_seg_flag
...
class LayoutParsingResultV2(BaseCVResult, HtmlMixin, XlsxMixin, MarkdownMixin):
    ...
    def _to_img(self) -> dict[str, np.ndarray]:
        ...
        # Handle both OCRResult objects and plain dicts (from DirectBlockOCR)
        if hasattr(self["overall_ocr_res"], 'img'):
            res_img_dict["overall_ocr_res"] = self["overall_ocr_res"].img["ocr_res_img"]
        else:
            # DirectBlockOCR returns a plain dict - create OCR visualization manually
            from src.pp.pipelines.ocr.result import draw_box_txt_fine
            from src.pp.utils.visualisation.fonts import CYRILLIC_FONT, create_font
            import random
            import cv2

            ocr_res = self["overall_ocr_res"]
            if "rec_polys" in ocr_res and len(ocr_res["rec_polys"]) > 0:
                boxes = ocr_res["rec_polys"]
                txts = ocr_res.get("rec_texts", [])
                ...
                # Use Cyrillic font for Ukrainian/Russian text support
                vis_font = CYRILLIC_FONT

                for idx, (box, txt) in enumerate(zip(boxes, txts)):
                    try:
                        color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
                        box_np = np.array(box)
                        box_pts = [(int(x), int(y)) for x, y in box_np.tolist()]
                        draw_left.polygon(box_pts, fill=color)

                        # Calculate box size
                        box_height = int(np.sqrt((box_np[0][0] - box_np[3][0])**2 + (box_np[0][1] - box_np[3][1])**2))
                        box_width = int(np.sqrt((box_np[0][0] - box_np[1][0])**2 + (box_np[0][1] - box_np[1][1])**2))

                        # Use minimum font size of 24 for better readability
                        font = create_font(txt, (box_width, box_height), vis_font.path, min_font_size=24)

                        # Create text image with better scaling
                        ...
                        pts2 = np.array(box_np, dtype=np.float32)
                        ...
                        pts = np.array(box_np, np.int32).reshape((-1, 1, 2))
                ...
                res_img_dict["overall_ocr_res"] = img_show
    ...
    def _to_json(self, *args, **kwargs) -> dict[str, str]:
        ...
        # Handle both OCRResult objects and plain dicts (from DirectBlockOCR)
        if hasattr(self["overall_ocr_res"], 'json'):
            data["overall_ocr_res"] = self["overall_ocr_res"].json["res"]
        else:
            # DirectBlockOCR returns a plain dict - use it directly
            data["overall_ocr_res"] = self["overall_ocr_res"]
        if model_settings["use_table_recognition"] and len(self["table_res_list"]) > 0:
            data["table_res_list"] = []
            for sno in range(len(self["table_res_list"])):
                table_res = self["table_res_list"][sno]
